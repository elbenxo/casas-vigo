...
/** Days threshold for "available soon" badges (used by FlatCard + RoomCard) */
export const SOON_DAYS = 30;

/** Sort rooms: available first, then by nearest available_from, then occupied without date */
export function sortRoomsByAvailability(rooms: Room[]): Room[] {
...
const avail = (availabilityData as { rooms: Record<string, { available: boolean; price: number; available_from: string | null }> }).rooms;
for (const flat of flats) {
  for (const room of flat.rooms) {
    ...
    if (data) {
      ...
      room.available_from = data.available_from ?? null;
    } else {
