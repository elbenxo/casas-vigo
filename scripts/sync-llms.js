...
function renderLlms({ flats, roomsByFlat }) {
  ...
  lines.push('');
  lines.push('## Vigo Family Guide (Things to Do)');
  lines.push('');
  lines.push(`Family travel guide for visitors staying in Vigo, especially near Samil beach. Available in 8 languages: [Vigo with kids](${SITE}/vigo/familias/)`);
  lines.push('');
  lines.push('- Samil beach with free seawater pools, playgrounds, Museo do Mar (sea museum + aquarium) and Verbum interactive museum');
  lines.push('- Cíes Islands — Atlantic Islands National Park, reached by ferry from Vigo (advance authorization + ticket required)');
  lines.push('- In the city: Monte do Castro fortress and views, Castrelos park, VigoNature nature/adventure centre, Marikiná Park treetop adventure, MARCO art museum');
  lines.push('- Day trips (<45 min): Baiona and the Pinta caravel replica, A Guarda and the Santa Trega Celtic hillfort, Pontevedra old town, Barosa waterfalls, wild Atlantic horses of Serra da Groba');
  lines.push('- Traditional Galician furanchos (seasonal home wineries): Almeriz, Reboraina, Fermín, Villa Preciosa');
  lines.push('');
...
function renderLlmsFull({ flats, roomsByFlat, generatedAt }) {
  ...
  lines.push('---');
  lines.push('');
  lines.push('## Vigo Family & Tourism Guide');
  lines.push('');
  lines.push(`A multilingual guide (8 languages) for families and visitors staying in Vigo, especially near Samil beach: ${SITE}/vigo/familias/`);
  lines.push('');
  lines.push('### Near Samil (walking distance)');
  lines.push('- Samil beach (1.7 km promenade, playgrounds) and free municipal seawater pools (open ~late May–September)');
  lines.push('- Museo do Mar de Galicia (sea museum with aquarium)');
  lines.push('- Verbum – Casa das Palabras (interactive language museum)');
  lines.push('- O Vao and A Fontaíña beaches (Blue Flag, calm waters)');
  lines.push('');
  lines.push('### In the city (<20 min)');
  lines.push('- Monte do Castro (fortress, panoramic views over the Ría)');
  lines.push('- Parque de Castrelos and Pazo Quiñones de León (free museum)');
  lines.push('- VigoNature (former Vigozoo, now a nature and adventure centre with zip lines)');
  lines.push('- Marikiná Park (treetop adventure park, ages 7+)');
  lines.push('- MARCO (contemporary art museum, free entry)');
  lines.push('');
  lines.push('### Star day trip: Cíes Islands');
  lines.push('- Atlantic Islands National Park; ferry from Vigo (~45 min). Requires regional authorization plus ferry ticket (Mar de Ons / Nabia). Return ferry ~27.50 EUR adult, ~12 EUR child.');
  lines.push('');
  lines.push('### Day trips (<45 min)');
  lines.push('- Baiona and the sailable Pinta caravel replica');
  lines.push('- A Guarda and the Santa Trega Celtic hillfort');
  lines.push('- Cangas (by ferry across the Ría)');
  lines.push('- Pontevedra pedestrian old town and Illa das Esculturas');
  lines.push('- Barosa waterfalls (Barro)');
  lines.push('- Serra da Groba — wild Atlantic horses roaming free');
  lines.push('');
  lines.push('### Local tradition: Furanchos');
  lines.push('Seasonal Galician home wineries serving house wine and simple home-cooked food, typically open in spring: Almeriz, Reboraina, Fermín, Villa Preciosa. Opening depends on the season — check locally.');
  lines.push('');
  lines.push('---');
  lines.push('');
  ...
  lines.push(`- Vigo Family Guide (things to do): ${SITE}/vigo/familias/`);
